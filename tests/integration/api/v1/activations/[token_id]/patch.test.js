...
describe("PATCH /api/v1/activations/[token_id]", () => {
  describe("Anonymouns user", () => {
    ...
    test("With expired token", async () => {
      jest.useFakeTimers({
        now: new Date(Date.now() - activation.EXPITATION_IN_MILLISECONS),
      });
    ...
    test("With valid token", async () => {
      ...
      expect(responseBody.updated_at < responseBody.created_at).not.toBe(true);
      ...
      expect(expiresAt - createdAt).toBe(activation.EXPITATION_IN_MILLISECONS);
